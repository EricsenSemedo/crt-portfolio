...
| `bun run build`    | Production build (TypeScript + Vite) to `dist/` |
| `bun run lint`     | Run ESLint across the project      |
| `bun run preview`  | Preview production build locally   |

**Testing:** Vitest is configured with jsdom. Run `bun run vitest run` to execute tests.

**Type-checking:** TypeScript errors surface during `bun run build` (via `vite build`). There is no standalone `tsc --noEmit` script; run `bunx tsc --noEmit` manually to type-check without building.
