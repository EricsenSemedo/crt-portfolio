...
| `yarn test`        | Run Vitest tests (single run)      |
...
- **Single service**: This is a frontend-only SPA. The only service is the Vite dev server (`yarn dev --host 0.0.0.0 --port 5173`). No backend, database, or Docker required.
- **Base path**: The app is configured with base `/crt-portfolio/`, so the dev server redirects `/` to `/crt-portfolio/`. Always navigate to `http://localhost:5173/crt-portfolio/` when testing.
- **Dev commands**: See the "Build / Lint / Test Commands" table above. All commands use `yarn`.
- **Vitest**: Tests exist at `src/components/__tests__/` using jsdom environment. Run `yarn test` for a single run.
